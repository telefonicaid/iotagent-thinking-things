...
function blackButtonResponse(parsedBody, req) {
    ...
    if (req.configurationValues) {
        ...
        result = (req.configurationValues.op_result && req.configurationValues.op_result.value.extra) ?
            req.configurationValues.op_result.value.extra : '';
    }
...
function manageIncomingResponse(id, value) {
    ...
    if (data && data.request && value && value.length === 1 && value[0].name && value[0].name === 'op_result') {
        ...
        data.request.configurationValues.op_result = value[0];
        delete synchronousRequests[id];

        if (value[0].value.code !== '200') {
            data.request.cbError = {
                details: {
                    code: value[0].value.code,
                    reasonPhrase: value[0].value.extra
                }
            };
        }
