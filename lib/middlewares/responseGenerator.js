    ...
    if (req.configurationValues) {
        ...
        if (req.configurationValues.op_result) {
            if (typeof req.configurationValues.op_result.value === 'string') {
                try {
                    parsedValue = JSON.parse(decode(req.configurationValues.op_result.value));

                    result = (parsedValue) ? parsedValue.extra : '';
                } catch (e) {
                    logger.warn(context, 'Parse error decoding result:\n%s\n', req.configurationValues.op_result.value);
                }
