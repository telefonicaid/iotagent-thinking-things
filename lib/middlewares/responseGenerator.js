...
function manageIncomingResponse(id, value) {
    ...
    if (data && data.request) {
        if (!data.request.configurationValues) {
            data.request.configurationValues = {};
        }
        ...
        logger.warning(
            context,
            'Incoming response not associated with previous synchronous Black Butotn operations for id [%s]. Ignoring',
            id
        );
    }
}
