...
var logger = require('logops'),
    ...
    errorHandler = require('./responseGenerator').errorHandler,
...
function parseResponse(req, res, next) {
    var payload = req.body.cadena || req.body.c || req.body.m;
...
function processNGSIError(error) {
    var errorObj;

    if (error && error.code === 200) {
        errorObj = error;
        errorObj.code = error.details.code || error.code;
    } else if (error) {
        errorObj = {
            code: error.code,
            details: {
                reasonPhrase: error.message || 'Unknown error'
            }
        };
    }
...
function updateValuesInCB(req, res, next) {
    iotAgentLib.update(req.entityId, req.type, '', req.compoundValues, req.deviceInformation, function(error) {
        if (error) {
            req.cbError = processNGSIError(error);
        }
...
function extractDeviceInformation(req, res, next) {
    ...
    if (req.deviceInformation) {
        ...
        req.entityId = config.ngsi.defaultType + ':' + req.entityId;
