...
function updateValuesInCB(req, res, next) {
        iotAgentLib.update(req.entityId, req.type, '', req.compoundValues, req.deviceInformation, next);
}

function retrieveConfiguration(req, res, next) {
    function extractName(item) {
        if (item.attributes && item.attributes.length === 1 && item.attributes[0].name) {
    ...
    if (req.configurations && req.configurations.length > 0) {
        ...
        var attributes = req.configurations.map(extractName);
